...
class MakeInvoice(models.TransientModel):
    _name = 'servicio.consolidado.crea.factura'
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(MakeInvoice, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar,submenu=submenu)
        product_ids = self._context.get('active_ids')
        product_lines = products_obj.browse(product_ids)
        partner_to_invoice = list(set(x.partner_invoice_id.id for x in product_lines))
        if len(partner_to_invoice) > 1:
            raise Warning('No puede elegir mas de un producto con diferentes clientes a facturar')


        return res

    @api.onchange('advance_payment_method')
    def set_partner_inoice_id(self):
        products_obj = self.env['producto.servicio.camion']
        ids = self._context['active_ids']
        product_lines = products_obj.browse(ids)
        self.partner_invoice_id = product_lines[0].partner_invoice_id.id
    ...
    def calcular_diario(self, partner_id):
        ...
        if partner_id.vat_type == '4' and partner_id.country_id.code != 'UY':
            # e-Ticket
    ...
    def facturar_regimen_transito_in(self, product_lines):
        if self.advance_payment_method == 'all':
            raise Warning('No se puede facturar entero un Transito In')
        account_obj = self.env['account.account']
        tax_obj = self.env['account.tax']
        camion_obj = self.env['carpeta.camion']
        operation_taxes = {
            'exento': tax_obj.search([('name', '=', 'Ventas Exentos IVA')]),
            'asimilado': tax_obj.search([('name', '=', 'IVA Venta asimilado a exportación')]),
            'gravado': tax_obj.search([('name', '=', 'IVA Ventas (22%)')])
        }
        lineas = []
        if product_lines:
            for line in product_lines:
                ...
                id_carga_camion = line.rt_carga_id.camion_id.id
                camion = camion_obj.browse(id_camion if id_camion else id_carga_camion)
                account_inter = account_obj.search([('code', '=', '41031007')])
                account_nat = account_obj.search([('code', '=', '41031008')])
                account_consol = account_obj.search([('code', '=', '41031009')])
                taxes = operation_taxes['asimilado']
                line_dict = {}
                    ...
                    if self.tramo_a_facturar == 'national':
                        if line.tramo_nat:
                            ...
                            if line.product_id.name == 'Flete':
                                ...
                                line_dict['account_id'] = account_nat.id
                                line_dict['price_unit'] = line.importe * self.amount / 100
                                line_dict['uom_id'] = line.product_id.uom_id.id
    ...
    def facturar_regimen_impo(self, product_lines):
        ...
        if product_lines:
            for line in product_lines:
                ...
                id_carga_camion = line.rt_carga_id.camion_id.id
                camion = camion_obj.browse(id_camion if id_camion else id_carga_camion)
                account_inter = account_obj.search([('code', '=', '41031003')])
                account_nat = account_obj.search([('code', '=', '41031004')])
                account_consol = account_obj.search([('code', '=', '41031009')])
    ...
    def make_invoices(self):
        ...
        ids_linea_producto = self._context.get('active_ids')
        product_service = self.env['producto.servicio.camion'].browse(ids_linea_producto)
        self.partner_invoice_id = product_service[0].partner_invoice_id.id
        ...
        if not camion.currency_id:
            raise Warning('El camion no tiene asignada una moneda, si no se encuentra una tarifa para el cliente usar la generica con la moneda a Facturar')
        if self.partner_invoice_id:
            if self.partner_invoice_id == camion.partner_invoice_id:
                partner = self.partner_invoice_id
            else:
                partner = self.partner_invoice_id
        else:
            raise Warning('Falta el cliente a facturar')


        journal_id = self.calcular_diario(partner)
        if not journal_id:
            raise UserError(_('Please define an accounting sales journal for this company.'))
